function g = grad_nls_TDOA(X,x,r,s)
...
D = sum((x-s).^2,1);
for i=2:L
    t1 = t1 + ( -(x(1)-X(1,i))/sqrt(ds(i)) + (x(1)-s(1))/sqrt(D) ) * ( r(i)-ds(i)^(0.5)+D^(0.5) );
    t2 = t2 + ( -(x(2)-X(2,i))/sqrt(ds(i)) + (x(2)-s(2))/sqrt(D) ) * ( r(i)-ds(i)^(0.5)+D^(0.5) );
end
g=2.*[t1; t2];
